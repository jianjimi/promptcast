...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let state = DbState::open(&db_path)
                .expect("init db");
            app.manage(state);
            ...
            // 3) 注册全局快捷键（若用户已设置）
            if let Ok(settings) = crate::db::settings::get_all(
                &app.state::<DbState>().0.lock(),
            ) {
                if let Some(shortcut) = settings.hotkey {
...
fn register_global_hotkey(app: &tauri::AppHandle, shortcut: &str) {
    ...
    let _ = app.global_shortcut().on_shortcut(shortcut, move |_app, _sc, ev: ShortcutEvent| {
